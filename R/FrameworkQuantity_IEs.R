...
get_IE <- function(model_fit,
  ...
  if (model[["type"]] %in% c("GLM", "GLMM")) {
    ...
    linear_predictor <- make_linear_predictor(
      mod = model, reg_of_interest = reg_of_interest, separate_interactions = FALSE
    )
    ...
    if (distribution == "empirical") {
      ...
      if (continue_metric) {
        result <- simple_emp_int(
          data = EmpDat, coef_draws = coef_draws, f = eval_g_theta_at_point
        )
      }
      ...
      if (continue_categorical) {
        ...
        RIvals_prep <- dealing_with_catRI(
          dat = EmpDat, g_theta = eval_g_theta_at_point, RIname = reg_of_interest
        )
